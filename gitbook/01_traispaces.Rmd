...
my_loadings <- apply(my_traitspace, 2, var)/sum(apply(my_traitspace, 2, var)) #this calculates the variance in each column and then divides it by the sum of all the variance values in all columns. this is used as 'percentage of variance explained in each dimension'
print(c(my_loadings[1], my_loadings[2], my_loadings[3]))
```
...
traits_matrix <- do.call(rbind, discrete_traits) #do.call is similar to apply, in this case it applies rbind to discrete_traits to make a matrix
```

There is one issue - the first step in calculating a PCA is centering the matrix, but you can't center when there's non-numeric values (?, 0/3, etc) what do we do? We need to make a distance matrix that has continuous values for each column.
...
distance_matrix2 <- char.diff(traits_matrix, method = "mord", by.col = FALSE,
                              ...
                              special.tokens = c("missing" = "\\?"), #\\ means "exactly this chacarter"
                              # defining the behaviour (always returning 1, the first state)
                              special.behaviour = list(missing = function(x,y) return(1))
                              ) #in this example, return(1) makes every iffy answer the same as the first state, which in this case is 0, probably not actually a good idea
```
...
my_pco <- cmdscale(distance_matrix,
                   # the number of dimensions, you remove the last one b/c it is ==0
                   k = ncol(distance_matrix)-2,
                   # the Cailliez correction, remove negative distances by adding a constant value across matrix
                   add = TRUE)
...
par(op)
```
In this case, its important to note that the first two coordinates only represent 5% of the variance - terrible for interpretation!
